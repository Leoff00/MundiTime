...
export default function Clock() {
  ...
  useEffect(() => {
    setInterval(() => {
      setSouthAfricaTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.SOUTH_AFRICA_TIMEZONE)
      );
      setSaudiArabiaTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.SAUDI_ARABIA_TIMEZONE)
      );
      setAustraliaTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.AUSTRALIA_TIMEZONE)
      );
      setBrazilTimeZone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.BRAZIL_TIMEZONE)
      );

      setCanadaTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.CANADA_TIMEZONE)
      );
      setChinaTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.CHINA_TIMEZONE)
      );
      setIvoryCoastTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.IVORY_COAST_TIMEZONE)
      );
      setEcuadorTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.ECUADOR_TIMEZONE)
      );

      setSpainTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.SPAIN_TIMEZONE)
      );
      setUsaTimezone(specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.USA_TIMEZONE));
      setGreeceTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.GREECE_TIMEZONE)
      );
      setIcelandTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.ICELAND_TIMEZONE)
      );

      setJapanTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.JAPAN_TIMEZONE)
      );
      setNewZelandTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.NEW_ZELAND_TIMEZONE)
      );
      setPortugalTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.PORTUGAL_TIMEZONE)
      );
      setRussiaTimezone(
        specifyDate(OPTIONS.DEFAULT_LOCALE, OPTIONS.RUSSIA_TIMEZONE)
      );
    }, 1000);
  }, []);
